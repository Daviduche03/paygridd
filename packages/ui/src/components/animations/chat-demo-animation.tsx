...
const NOTIFICATION_MP3 = "https://cdn.paygrid.xyz/notification.mp3";
const KEYBOARD_MP3 = "https://cdn.paygrid.xyz/keyboard.mp3";
    ...
    steps: [
      {
        ...
        attachment: {
          ...
          amount: "SEK 1,018.35",
          imageSrc: "https://cdn.paygrid.xyz/chat-receipt-hemkop.png",
        },
        delayAfterMs: 700,
      },
      {
...
const HOLD_PAYGRID_MESSAGE = 850;
  ...
  for (let si = 0; si < scenarioCount; si++) {
    ...
    if (config.startOnLockScreen) {
      ...
      for (let stepIdx = 1; stepIdx < config.steps.length; stepIdx++) {
        ...
        if (step.sender === "paygrid" && step.typingMs) {
        ...
        snap(step.sender === "user" ? HOLD_USER_MESSAGE : HOLD_PAYGRID_MESSAGE);
      ...
      if (firstStep) {
        ...
        if (firstStep.sender === "paygrid" && firstStep.typingMs) {
          snap(HOLD_TYPING, { isTyping: true });
        }
        ...
        snap(
          firstStep.sender === "user" ? HOLD_USER_MESSAGE : HOLD_PAYGRID_MESSAGE,
        );
...
function IOSKeyboard() {
  return (
    <img
      src="https://cdn.paygrid.xyz/ios-keyboard-dark-v1.svg"
      alt=""
      draggable={false}
...
/* ------------------------------------------------------------------ */
/*  PayGrid logo (inlined SVG, white on black)                          */
/* ------------------------------------------------------------------ */

function PayGridLogo({
  size,
  borderRadius,
}: {
  size: number;
...
const LOCK_SCREEN_WALLPAPER = "https://cdn.paygrid.xyz/background-remote-v7.png";
...
function NotificationBanner({
  ...
  return (
    ...
    >
      ...
      >
        <PayGridLogo size={38} borderRadius={9} />
...
const RECEIPT_IMAGE_SRC = "https://cdn.paygrid.xyz/reciept.jpg";
    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <a
            href="https://cal.com/pontus-paygrid/15min"
            target="_blank"
