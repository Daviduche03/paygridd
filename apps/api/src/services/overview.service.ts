...
function buildEmptySummary(currency: string) {
  return {
    ...
    unbilledTime: { totalDuration: 0, totalAmount: 0, projectCount: 0, currency },
...
export const overviewService = {
  async getSummary(userId: string) {
    ...
    const [openInvoices, customersCount, accountsSummary, pendingReviewCount, balance] =
      await Promise.all([
    ...
    const currency = openInvoices.currency || accountsSummary.currency || baseCurrency;
