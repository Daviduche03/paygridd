...
import type { SyncConnectionPayload } from "jobs/schema";
...
export const manualSyncTransactionsAction = authActionClient
  ...
  .action(async ({ parsedInput: { connectionId }, ctx: { businessId } }) => {
    // Verify the connection belongs to the caller's business
