...
import { useStableInfiniteQuery } from "@/hooks/use-stable-query";
...
export function DataTable({ initialSettings, onSelectCustomer }: Props) {
  const trpc = useTRPC();
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, refetch, isPending, isError, error } =
    useStableInfiniteQuery(infiniteQueryOptions);
  ...
  useRealtime({
    ...
    table: "customers",
    filter: user?.businessId ? `business_id=eq.${user.businessId}` : undefined,
    onEvent: (payload) => {
  ...
  if (isError) {
    return (
      <EmptyState
        ...
        description={error?.message ?? "Check that the API is running and you are signed in."}
      />
    );
  }
