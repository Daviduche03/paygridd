...
import type { InvoiceProduct } from "invoice/types";
...
export function ProductAutocomplete({
  ...
  const filteredProducts = (allProducts ?? []).filter((product) => {
    if (parsedValue.trim().length < 2) return true; // Show all when not searching
    return product.name.toLowerCase().includes(parsedValue.toLowerCase());
  }).slice(0, parsedValue.trim().length >= 2 ? undefined : 5); // Limit to 5 when not searching
