...
export function BankAccountList() {
  ...
  const { data: connections, isPending: isConnectionsPending } = useStableQuery(
    trpc.bankConnections.get.queryOptions(),
  );

  const { data: manualAccounts, isPending: isManualAccountsPending } =
    useStableQuery(
  ...
  if (
    (isConnectionsPending && !connections) ||
    (isManualAccountsPending && !manualAccounts)
  ) {
